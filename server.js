...
// Dossier public qui contient index.html
...
app.delete('/nettoyer-perimes', async (req, res) => {
    ...
    const restants = db.data.produits.filter(
        p => p.dateExpiration >= ajd
    );

    await db.update(data => {
        data.produits = restants;
    });
...
app.listen(3050, '0.0.0.0', () => {
    console.log(
        'Serveur Pro actif sur http://127.0.0.1:3050'
    );
});
